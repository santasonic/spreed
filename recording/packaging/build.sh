...
# "build/{DISTRIBUTION_ID}/{PACKAGE FORMAT}/" directory under "packaging" with
...
--container CONTAINER_NAME the name (prefix) to assign to the containers.
  Defaults to talk-recording-packages-builder."
if [ "$1" = "--help" ]; then
	echo "$HELP"
...
CONTAINER="talk-recording-packages-builder"
if [ "$1" = "--container" ]; then
	CONTAINER="$2"
...
if [ -z "$(docker ps --all --quiet --filter name="^/$CONTAINER-ubuntu20.04$")" ]; then
	echo "Creating Talk recording packages builder container for Ubuntu 20.04"
...
if [ -z "$(docker ps --all --quiet --filter name="^/$CONTAINER-ubuntu22.04$")" ]; then
	echo "Creating Talk recording packages builder container for Ubuntu 22.04"
